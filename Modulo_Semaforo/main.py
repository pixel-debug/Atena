...
try:
	for frame in frames.capture_continuous(capturaFrames, format="bgr", use_video_port=True):
		...
		SINAL_VERMELHO, SINAL_VERDE = trata.sinalizacao_direita(imagem_sinalizacao_dir)
		...
		if(FAIXA_CONTENCAO_FOTO is True):	
			...
			if(SINAL_VERMELHO is True):
				...
				while(SINAL_VERMELHO is True):
					SINAL_VERMELHO, _ = trata.sinalizacao_direita(imagem_sinalizacao_dir)
					gerencia.interrupcao_movimento(ctr_vel_motor_dir, ctr_vel_motor_esq)
					if(SINAL_VERMELHO is False):
						SINAL_VERMELHO = False
			if(SINAL_VERMELHO is False):
				print("Sinal Verde! Prosseguir ate sair da faixa...")
