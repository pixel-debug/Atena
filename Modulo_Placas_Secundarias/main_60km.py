...
try:
	for frame in frames.capture_continuous(capturaFrames, format="bgr", use_video_port=True):
		...
		# -------------------------- Condicionais Placas---------------------------------
		if(status_placa_60km is True):
			print("Detectou placa 60 km/h!")
			while((tempoDeteccao < var.CONST_TEMPO_PLC_60KM)):
				status_placa_60km = trata.sinalizacao_direita(imagem_sinalizacao_dir)
				print("Aumentando velocidade...")
				gerencia.movimento_frente(var.velAumentada, ctr_vel_motor_dir, ctr_vel_motor_esq)
			else:
				status_placa_60km = False
