...
try:
	for frame in frames.capture_continuous(capturaFrames, format="bgr", use_video_port=True):
		...
		status_museu, status_igreja, status_teatro = trata.sinalizacao_esquerda(imagem_sinalizacao_esq)
		print(ck1, ck2, ck3, status_museu, status_igreja,  status_teatro)
		...
		if((ck2 is True) and (status_igreja is True)):  
			print("Você chegou no destino {0}. Desligando...".format(var_hsv.nome_check_2))
			gerencia.interrupcao_movimento(ctr_vel_motor_dir, ctr_vel_motor_esq)
			break
		#if((ck1 is True) and (status_museu is True)):
		#	print("Você chegou no destino {0}. Desligando...".format(var_hsv.nome_check_1))			
		#	break
		#if((ck3 is True) and (status_teatro is True)):
		#	print("Você chegou no destino {0}. Desligando...".format(var_hsv.nome_check_3))
		#	break
		...
		if(FAIXA_CONTENCAO_VISAO is True):
			...
			print("Detectou faixa de pedestre a frente!")
		...
		#cv2.imshow("Sinalizacoes da Esquerda", imagem_sinalizacao_esq)
		# -------------------------- Apresentacao Telas ---------------------------------
